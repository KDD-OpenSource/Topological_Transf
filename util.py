...
STDEVS = {
    ...
    'fashion_mnist' : {'fgsm': 0.271, 'bim-a': 0.111, 'bim-b': 0.167, 'cw-l2': 0.207}, #TODO finetune- understand and do
    'Animals_10' :  {'fgsm': 0.271, 'bim-a': 0.111, 'bim-b': 0.167, 'cw-l2': 0.207} ,
    'PetImages' :  {'fgsm': 0.271, 'bim-a': 0.111, 'bim-b': 0.167, 'cw-l2': 0.207}
}
...
def get_data(dataset='mnist'):
    ...
    :return: 
    """
    assert dataset in ['mnist', 'cifar', 'svhn', 'fashion_mnist', 'PetImages', 'Animals_10'], \
    ...
    if dataset == 'mnist':
    ...
    elif dataset == 'Animals_10':
        set = CatDogDataset('../loads_cats_dogs/Animals_10/', cat_dir='cat/', dog_dir='dog/', samplesize=5000) #TODO Specify path of Dataset
        train_set, test_set, train_set_len, test_set_len = split_trainings_data_into_train_and_val(set, batchsize=500) # TODO assert batchsize
        ...
        set = CatDogDataset('../loads_cats_dogs/PetImages/', cat_dir='Cat/', dog_dir='Dog/', samplesize=5000)
        train_set, test_set, train_set_len, test_set_len = split_trainings_data_into_train_and_val(set, batchsize=500) # TODO assert batchsize
        for i, batch in enumerate(train_set, 0): #TODO change that batchsize can be smaller
            inputs, labels = batch
        X_train = inputs.numpy()
    ...
    if dataset == 'PetImages' or dataset == 'Animals_10':
        Y_train = np_utils.to_categorical(y_train, 2) # TODO eval expects 10 classes? why?
...
def get_model(dataset='mnist', softmax=True): # TODO hp pass hyperparameters
    """
    ...
    """
    assert dataset in ['mnist', 'cifar', 'svhn', 'fashion_mnist', 'Animals_10', 'PetImages'], \
    ...
    if dataset == 'mnist':
        ...
        layers = [
            Conv2D(64, (3, 3), padding='valid', input_shape=(224, 224, 3)),  # 0
            Activation('relu'),  # 1
            BatchNormalization(),  # 2
            Conv2D(64, (3, 3)),  # 3
            Activation('relu'),  # 4
